...
void ICACHE_FLASH_ATTR coap_dumpHeader(coap_header_t *hdr)
{
    ...
    os_printf("  t    0x%02X\n", hdr->t);
    os_printf("  tkl  0x%02X\n", hdr->tkl);
    os_printf("  code 0x%02X\n", hdr->code);
    os_printf("  id   0x%02X%02X\n", hdr->id[0], hdr->id[1]);
}
#endif
...
const ICACHE_FLASH_ATTR coap_option_t *coap_findOptions(const coap_packet_t *pkt, uint8_t num, uint8_t *count)
{
